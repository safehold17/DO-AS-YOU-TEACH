...
class TD3(object):
    ...
    def train(self, replay_buffer, batch_size=256, alice = None, agent = "alice"):
        ...
        grid, state, action, reward, next_grid, next_state, start, not_done = zip(*batch)
        ...
        start = torch.FloatTensor(start).to(device)
        ...
        if self.total_it % self.policy_freq == 0:
            ...
            if agent == "bob":
                # print(state.shape)
                alice_actions = alice.TD.actor(torch.cat((state[:,:7], start), dim=1), grid[:,:2,:,:])
